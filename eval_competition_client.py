...
from neural_decoder.decode_server_utils import DecodeServerClient, add_decode_server_args
...
parser = argparse.ArgumentParser(
    description="Run evaluation using a persistent local decode server plus local/Modal/OpenAI reranking."
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Optional cap on how many utterances to process. Use 0 for the full split.",
)
...
def compute_total_utterances(loaded_data, partition_name, max_utterances=0):
    total = sum(len(session["transcriptions"]) for session in loaded_data[partition_name])
    if max_utterances > 0:
        return min(total, max_utterances)
    return total
...
decode_client = DecodeServerClient(
    host=input_args.decodeServerHost,
    port=input_args.decodeServerPort,
    authkey=input_args.decodeServerAuthkey,
)
server_info = decode_client.health()
print(
    f"Connected to decode server at {input_args.decodeServerHost}:{input_args.decodeServerPort} "
    f"using lmDir={server_info['lm_dir']}",
    flush=True,
)
...
total_utterances = compute_total_utterances(
    loadedData,
    partition,
    max_utterances=input_args.maxUtterances,
)
        ...
        for i, testDayIdx in enumerate(partition_day_indices):
            ...
            for j, (X, y, X_len, y_len, _) in enumerate(test_loader):
                ...
                for iterIdx in range(pred.shape[0]):
                    if submitted_count >= total_utterances:
                        stop_requested = True
                        break
                    ...
                    raw_nbest = decode_client.decode_nbest(
                        logits[0],
